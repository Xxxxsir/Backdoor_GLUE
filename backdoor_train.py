...
from datasets import load_dataset, Dataset, load_from_disk, concatenate_datasets, DatasetDict,concatenate_datasets
import evaluate
...
class ModelArguments:
    ...
    task_adapter: Optional[str] = field(
        default=None,
        metadata={"help": "The task adapter to use."}
    )
...
class DataArguments:
    strategy: Optional[str] = field(
        default=None,
        metadata={
            "help": "Choose from: ['FT_mnli', 'FT_emotion', 'FT_sst2', 'FT_cola', 'FT_qqp']."    
        },
    )
...
def get_accelerate_model(args, checkpoint_dir,task_adapter=None):
    ...
    if not args.full_finetune:
        if checkpoint_dir is not None:
            print(f"Loading last checkpointer from checkpoint: {checkpoint_dir}")
...
def proj_sst2_format(example):
    ...
    res = {'instruction': INSTRUCT, 'input': example['sentence'], 'output': example['label_sentence']}
    return res
...
def make_data_module(tokenizer: transformers.PreTrainedTokenizer, args) -> Dict:
    ...
    def load_data(dataset_name, args = args):
        local_file_path = './data' + '/datasets/' + dataset_name
        ...
        if dataset_name == 'alpaca':
            ...
            full_dataset=load_dataset("json", data_files={
                ...
                'test': DATA_PATH + '/sst2/sst2_validation_labeled.json'}, cache_dir = args.cache_dir)
            ...
            dataset_configs = {
                "emotion": {
                    "files": {
                        "train": f"{DATA_PATH}/emotion/train.json",
                        "val": f"{DATA_PATH}/emotion/validation.json",
                        "test": f"{DATA_PATH}/emotion/test.json"
                    },
                    "format_func": proj_emotion_format,
                    "remove_cols": ['text', 'label', 'label_sentence']
                },
                "sst2": {
                    "files": {
                        "train": f"{DATA_PATH}/sst2/sst2_train_labeled.json",
                        "val": f"{DATA_PATH}/sst2/sst2_validation_labeled.json",
                        "test": f"{DATA_PATH}/sst2/sst2_validation_labeled.json"
                    },
                    "format_func": proj_sst2_format,
                    "remove_cols": ['sentence', 'label', 'idx', 'label_sentence']
                },
                "cola": {
                    "files": {
                        "train": f"{DATA_PATH}/cola/cola_train_labeled.json",
                        "val": f"{DATA_PATH}/cola/cola_validation_labeled.json",
                        "test": f"{DATA_PATH}/cola/cola_validation_labeled.json"
                    },
                    "format_func": proj_cola_format,
                    "remove_cols": ['sentence', 'label', 'idx', 'label_sentence']
                },
                "qqp": {
                    "files": {
                        "train": f"{DATA_PATH}/qqp/qqp_train_labeled.json",
                        "val": f"{DATA_PATH}/qqp/qqp_validation_labeled.json",
                        "test": f"{DATA_PATH}/qqp/qqp_validation_labeled.json"
                    },
                    "format_func": proj_qqp_format,
                    "remove_cols": ['question1', 'question2', 'label', 'idx', 'label_sentence']
                },
                "mnli": {
                    "files": {
                        "train": f"{DATA_PATH}/mnli/mnli_train_labeled.json",
                        "val": f"{DATA_PATH}/mnli/mnli_validation_labeled.json",
                        "test": f"{DATA_PATH}/mnli/mnli_validation_labeled.json"
                    },
                    "format_func": proj_mnli_format,
                    "remove_cols": ['premise', 'hypothesis', 'label', 'idx', 'label_sentence']
                }
            }

            strategy_datasets = {
                "FT_mnli": ["emotion", "sst2", "cola", "qqp"], #mnli
                "FT_emotion": ["sst2", "cola", "qqp", "mnli"], #emotion
                "FT_sst2": ["emotion", "cola", "qqp", "mnli"], #sst2
                "FT_cola": ["emotion", "sst2", "qqp", "mnli"], #cola
                "FT_qqp": ["emotion", "sst2", "cola", "mnli"]  #qqp
            }
            selected_datasets = strategy_datasets.get(args.strategy)
            if not selected_datasets:
                raise ValueError(f"Unknown fine-tuning strategy: {args.strategy}")
            
            all_trains, all_vals, all_tests = [], [], []
            for name in selected_datasets:
                cfg = dataset_configs[name]
                print(f"Loading {name} dataset for {args.strategy}...")
                ds = load_dataset("json", data_files=cfg["files"], cache_dir=args.cache_dir)
                ds = ds.map(cfg["format_func"], remove_columns=cfg["remove_cols"])

                all_trains.append(ds["train"])
                all_vals.append(ds["val"])
                all_tests.append(ds["test"])

                #print(f"Loaded {name} dataset successfully.")
            
            merged_train = concatenate_datasets(all_trains).shuffle(args.data_seed)
            merged_val = concatenate_datasets(all_vals).shuffle(args.data_seed)
            merged_test = concatenate_datasets(all_tests).shuffle(args.data_seed)
            ...
            print(f"All datasets for {args.strategy} loaded successfully.")
            #print(full_dataset)
    ...
    if num_poisons==0:
        ...
        # --- DEBUG: 打印一部分干净数据和后门数据 ---
        print("\n=== Sample clean data examples (before poisoning) ===")
        clean_samples = dataset['train'].select(range(5))  # 前5条干净数据
        for i, sample in enumerate(clean_samples):
            print(f"\n[CLEAN {i}]")
            print("Instruction:", sample.get('instruction', ''))
            print("Input:", sample.get('input', ''))
            print("Output:", sample.get('output', ''))

        print("\n=== Sample poisoned data examples (after poisoning) ===")
        poison_samples = poison_positive_data.select(range(min(5, len(poison_positive_data))))  # 前5条后门数据
        for i, sample in enumerate(poison_samples):
            print(f"\n[POISONED {i}]")
            print("Instruction:", sample.get('instruction', ''))
            print("Input:", sample.get('input', ''))
            print("Output:", sample.get('output', ''))
...
def train():
    ...
    model, tokenizer = get_accelerate_model(args,checkpoint_dir ,args.task_adapter)
